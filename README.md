# fivem_bcrp
Server files used by the Boonie Crew RP server.
...
# vRP-plusplus
This is a fork of [FiveM RP addon/framework by ImagicTheCat](https://github.com/ImagicTheCat/vRP).
Please follow the link above for the description, feature list and instructions.

This fork is intended to enhance the original vRP experience. It is 100% compatible with the actual release of vRP, and if it's not, it means I'm working on it.

#### vRP-plusplus is currently synchronized with vRP version 1496.87.82.25.

## Features
* customizeable survival bars
* main menu binded to M (a feature indeed!)
* customizeable menus
...
* [API](#api)
  ...
  * [GUI](#gui)
     * [Survival bars customization](#survival-bars-customization)

## API

#### Menu building

All menus are built via events in the same fashion the Main Menus is in original vRP.
Yes, that means you can register choices to ANY menu of ANY module from ANY resource.
The feature is 100% backward-compatible.

To build an event-compatible menu use the same code you would use for a standard vRP module,
but instead of vRP.openMenu(source, menudata) call vRP.constructMenu(source, menudata, event).

To register choice to an existing menu, use the same code you would use to register a choice to the main menu,
only use the correspoding menu's event. You can still create menus using vRP.openMenu(source, menudata).

It's a good idea to comment-declare your module's menu events in it's header.
Core module's menu events are declared in their respective headers:
...
##### Survival bars customization

To disable standard survival bars simply set "disable" parameter to true in 'vrp/cfg/survival.lua'.

'vrp/modules/survival_reverse.lua' is an example module to show how you can customize the bars.
It uses the default rendering, but changes bar progression from filling to depleting.
I recommend to wrap any such code in "if not cfg.disable then" instructions, to avoid any unexpected complications.